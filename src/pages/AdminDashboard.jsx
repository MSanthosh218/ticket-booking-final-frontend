...
import { useAuth } from '../context/AuthContext';
import { getAllUsers, getAllMovies, deleteMovie, deleteUser } from '../api';
...
const AdminDashboard = () => {
    ...
    return (
      <div className="container mx-auto p-4 sm:p-6">
        <div className="text-center py-6 sm:py-8">
          <div className="text-lg sm:text-xl">Loading Admin Dashboard...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="container mx-auto p-4 sm:p-6">
        <div className="text-center py-6 sm:py-8">
          ...
          {/* Only show retry if it's not an authorization error */}
          {error.includes('authorized') ? (
            <p className="text-gray-600 text-sm sm:text-base">Please ensure you are logged in as an ADMIN.</p>
          ) : (
            <button
              onClick={fetchData}
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6">
      <h1 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-8 text-center text-gray-800">Admin Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-3 sm:mb-4">User Management ({users.length})</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white border border-gray-200 text-left text-sm sm:text-base">
              <thead>
                <tr className="bg-gray-50">
                  <th className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Name</th>
                  <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Email</th>
                  <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Role</th>
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-gray-50">
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base text-gray-800">{u.name}</td>
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base text-gray-800">{u.email}</td>
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base text-gray-800">{u.role}</td>
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base">
                      <button onClick={() => handleDeleteUser(u.id)} className="text-red-600 hover:underline text-sm sm:text-base">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {/* Movie Management Section */}
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-3 sm:mb-4">Movie Management ({movies.length})</h2>
          <div className="flex justify-end mb-3 sm:mb-4">
            <Link to="/create-movie" className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 sm:px-4 sm:py-2 rounded text-sm sm:text-base">
              Add New Movie
            </Link>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white border border-gray-200 text-left text-xs sm:text-sm">
              <thead>
                <tr className="bg-gray-50">
                  <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Title</th>
                  <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Language</th>
                  <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Duration</th>
                  <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                {movies.map((m) => (
                  <tr key={m.id} className="hover:bg-gray-50">
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base text-gray-800">{m.title}</td>
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base text-gray-800">{m.language}</td>
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base text-gray-800">{m.duration} mins</td>
                    <td className="py-2 px-3 sm:px-4 border-b text-sm sm:text-base">
                      <Link to={`/edit-movie/${m.id}`} className="text-blue-600 hover:underline mr-1 sm:mr-2 text-xs sm:text-sm">Edit</Link>
                      <button onClick={() => handleDeleteMovie(m.id)} className="text-red-600 hover:underline text-xs sm:text-sm">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
