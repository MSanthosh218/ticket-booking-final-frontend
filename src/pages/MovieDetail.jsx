...
const MovieDetail = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-48 sm:h-64">
        ...
        <p className="ml-2 sm:ml-4 text-base sm:text-lg">Loading movie details...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center mt-6 sm:mt-8 p-3 sm:p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg text-sm sm:text-base">
        <p className="font-bold">Error:</p>
        <p>{error}</p>
        <button
          onClick={() => navigate('/')}
          className="mt-3 sm:mt-4 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-full transition duration-300 text-sm sm:text-base"
        >
          Back to Movies
        </button>
      </div>
  ...
  if (!movie) {
    return (
      <div className="text-center mt-6 sm:mt-8 p-3 sm:p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded-lg text-sm sm:text-base">
        <p>Movie not found.</p>
        <button
          onClick={() => navigate('/')}
          className="mt-3 sm:mt-4 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-full transition duration-300 text-sm sm:text-base"
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 bg-white rounded-xl shadow-2xl border border-gray-200">
      <button
        onClick={() => navigate('/')}
        className="mb-4 sm:mb-6 bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-4 sm:px-5 rounded-full transition duration-300 ease-in-out transform hover:scale-105 text-sm sm:text-base"
      >
        &larr; Back to All Movies
      </button>
      ...
      <div className="flex flex-col md:flex-row gap-6 sm:gap-8 items-start">
        <div className="md:w-1/3 w-full flex-shrink-0">
          <img
            src={movie.posterUrl || `https://placehold.co/400x600/E0E7FF/4F46E5?text=${encodeURIComponent(movie.title)}`}
            alt={movie.title}
            className="w-full h-auto rounded-lg shadow-lg object-cover border border-gray-300 max-h-96 md:max-h-[unset]"
            onError={(e) => {
              e.target.onerror = null;
              e.target.src = `https://placehold.co/400x600/E0E7FF/4F46E5?text=${encodeURIComponent(movie.title)}`;
            }}
        ...
        <div className="md:w-2/3 w-full">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-800 mb-2 sm:mb-4 leading-tight">
            {movie.title}
          </h1>
          <p className="text-base sm:text-lg text-gray-600 mb-3 sm:mb-4">
            {movie.language} | {movie.duration} mins | Released: {new Date(movie.releaseDate).toLocaleDateString()}
          </p>
          <p className="text-sm sm:text-base text-gray-700 leading-normal sm:leading-relaxed mb-4 sm:mb-6">
            {movie.description || 'No description available for this movie.'}
          </p>

          <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-4 sm:mb-5 border-b-2 pb-2 border-purple-300">
            Available Shows
          </h2>
          {shows.length === 0 ? (
            <p className="text-gray-600 text-base sm:text-lg">No shows available for this movie yet.</p>
          ) : (
            <div className="space-y-4 sm:space-y-5">
              {shows.map((show) => (
                <div key={show.id} className="bg-blue-50 p-4 sm:p-5 rounded-lg shadow-md border border-blue-200 flex flex-col sm:flex-row justify-between items-center transition-all duration-300 hover:shadow-lg text-center sm:text-left">
                  ...
                  <button
                    onClick={() => handleBookShow(show)}
                    className="mt-4 sm:mt-0 bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white font-semibold py-2 px-4 sm:py-3 sm:px-6 rounded-full shadow-lg transition duration-300 ease-in-out transform hover:scale-105 text-sm sm:text-base"
                  >
                    Book Tickets
                  </button>
                </div>
