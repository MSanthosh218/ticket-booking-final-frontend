...
const TheatrePage = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-48 sm:h-64">
        ...
        <p className="ml-2 sm:ml-4 text-base sm:text-lg">Loading theatres...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6">
      <h1 className="text-4xl sm:text-5xl font-extrabold text-center text-gray-800 mb-8 sm:mb-10 tracking-tight">
        Our Theatres
      </h1>
      {theatres.length === 0 ? (
        <p className="text-center text-lg sm:text-xl text-gray-600 p-6 sm:p-8 bg-white rounded-lg shadow-md">
          No theatres registered yet.
        </p>
      ) : (
          ...
          {theatres.map((theatre) => (
            <div key={theatre.id} className="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200 transition-transform duration-300 hover:scale-[1.02] hover:shadow-2xl">
              <div className="p-4 sm:p-6">
                ...
                <button
                  onClick={() => toggleScreens(theatre.id)}
                  className="mt-3 sm:mt-4 w-full bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-semibold py-2 px-4 sm:py-3 rounded-full shadow-md transition duration-300 ease-in-out transform hover:scale-105 text-sm sm:text-base"
                >
                  {expandedTheatreId === theatre.id ? 'Hide Screens' : 'View Screens'}
                </button>
                ...
                {expandedTheatreId === theatre.id && (
                  <div className="mt-4 sm:mt-6 border-t border-gray-200 pt-3 sm:pt-4">
                    <h3 className="text-lg sm:text-xl font-bold text-gray-700 mb-2 sm:mb-3">Screens:</h3>
                    {theatreScreens[theatre.id] && theatreScreens[theatre.id].length > 0 ? (
                      <ul className="space-y-1.5 sm:space-y-2">
                        {theatreScreens[theatre.id].map(screen => (
                          <li key={screen.id} className="bg-gray-50 p-2 sm:p-3 rounded-lg shadow-sm border border-gray-100">
                            <p className="font-semibold text-gray-800 text-sm sm:text-base">{screen.name}</p>
                            <p className="text-gray-600 text-xs sm:text-sm">Capacity: {screen.capacity}</p>
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <p className="text-gray-600 text-xs sm:text-sm">No screens found for this theatre.</p>
                    )}
                  </div>
                )}
              </div>
