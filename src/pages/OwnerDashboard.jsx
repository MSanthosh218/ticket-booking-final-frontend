...
// import { getOwnerTheatres, deleteTheatre, deleteScreen, deleteShow, getAllMovies } from '../api'; // Added getAllMovies
...
//       } else if (response && response.theatres && Array.isArray(response.theatres)) {
...
const OwnerDashboard = () => {
  ...
  const fetchOwnerData = async () => {
    try {
      ...
      // Handle different response formats (though `getOwnerTheatres` from backend should return an array directly now)
      let theatresData = [];
      if (Array.isArray(response)) {
        theatresData = response;
      } else if (response && response.data && Array.isArray(response.data)) {
