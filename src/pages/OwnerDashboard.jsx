...
const OwnerDashboard = () => {
    ...
    return (
      <div className="container mx-auto p-4 sm:p-6">
        <div className="text-center py-6 sm:py-8">
          <div className="text-lg sm:text-xl">Loading Owner Dashboard...</div>
          <div className="text-sm text-gray-500 mt-2">
            Auth: {isAuthenticated ? 'Yes' : 'No'} |
            User: {user ? user.role : 'None'} |
            Token: {token ? 'Present' : 'Missing'}
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6">
      <h1 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-8 text-center text-gray-800">Owner Dashboard</h1>

      {/* Debug Info */}
      <div className="bg-gray-100 p-3 sm:p-4 rounded mb-4 sm:mb-6 text-xs sm:text-sm">
        <strong>Debug Info:</strong> Theatres loaded: {ownerTheatres.length} |
        User: {user?.email} ({user?.role}) |
        Token: {token ? 'Present' : 'Missing'}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8 sm:mb-10">
        <Link to="/create-movie" className="bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 sm:py-3 sm:px-6 rounded-lg shadow-lg flex items-center justify-center text-center text-sm sm:text-base lg:text-lg">
        ...
        <Link to="/create-theatre" className="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 sm:py-3 sm:px-6 rounded-lg shadow-lg flex items-center justify-center text-center text-sm sm:text-base lg:text-lg">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6 mr-1 sm:mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
          </svg>
          Add Theatre
        </Link>
        <Link to="/create-screen" className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 sm:py-3 sm:px-6 rounded-lg shadow-lg flex items-center justify-center text-center text-sm sm:text-base lg:text-lg">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6 mr-1 sm:mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
          </svg>
          Add Screen
        </Link>
        <Link to="/create-show" className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 sm:py-3 sm:px-6 rounded-lg shadow-lg flex items-center justify-center text-center text-sm sm:text-base lg:text-lg">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6 mr-1 sm:mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
          </svg>
          Add Show
      ...
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 sm:mb-6">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-2 sm:mb-0">My Theatres ({ownerTheatres.length})</h2>
          <button
            onClick={fetchOwnerData}
            className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 sm:px-4 sm:py-2 rounded text-sm sm:text-base"
          >
            Refresh
          </button>
        </div>
        ...
        {ownerTheatres.length > 0 ? (
          <div className="space-y-6 sm:space-y-8">
            {ownerTheatres.map((theatre) => (
              <div key={theatre.id} className="border border-gray-200 p-4 sm:p-6 rounded-lg shadow-sm">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4">
                  <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-1 sm:mb-0">{theatre.name}</h3>
                  <div className="flex space-x-1 sm:space-x-2 mt-2 sm:mt-0">
                    <Link to={`/edit-theatre/${theatre.id}`} className="bg-blue-500 hover:bg-blue-600 text-white text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1 rounded">Edit</Link>
                    <button onClick={() => handleDeleteTheatre(theatre.id)} className="bg-red-500 hover:bg-red-600 text-white text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1 rounded">Delete</button>
                  </div>
                </div>
                <p className="text-gray-600 text-xs sm:text-sm mb-2">{theatre.location} | Total Seats: {theatre.totalSeats}</p>

                {/* Screens Section */}
                <div className="mt-4 sm:mt-6">
                  <h4 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Screens ({theatre.screens?.length || 0})</h4>
                  {theatre.screens && theatre.screens.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="min-w-full bg-white border border-gray-200 text-left text-xs sm:text-sm">
                        <thead>
                          <tr className="bg-gray-50">
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Name</th>
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Capacity</th>
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                          {theatre.screens.map((screen) => (
                            <tr key={screen.id} className="hover:bg-gray-50">
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm text-gray-800">{screen.name}</td>
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm text-gray-800">{screen.capacity}</td>
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm">
                                <Link to={`/edit-screen/${screen.id}`} className="text-blue-600 hover:underline mr-1 sm:mr-2 text-xs sm:text-sm">Edit</Link>
                                <button onClick={() => handleDeleteScreen(screen.id)} className="text-red-600 hover:underline text-xs sm:text-sm">Delete</button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    <p className="text-gray-600 text-xs sm:text-sm">No screens added to this theatre yet. <Link to="/create-screen" className="text-blue-600 hover:underline">Add one</Link>.</p>
                  )}
                </div>

                {/* Shows Section */}
                <div className="mt-4 sm:mt-6">
                  <h4 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Shows ({theatre.shows?.length || 0})</h4>
                  {theatre.shows && theatre.shows.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="min-w-full bg-white border border-gray-200 text-left text-xs sm:text-sm">
                        <thead>
                          <tr className="bg-gray-50">
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Movie</th>
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Screen</th>
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Time</th>
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Price</th>
                            <th className="py-2 px-3 sm:px-4 border-b text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                          {theatre.shows.map((show) => (
                            <tr key={show.id} className="hover:bg-gray-50">
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm text-gray-800">{movies[show.movieId] || 'N/A'}</td>
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm text-gray-800">{theatre.screens.find(s => s.id === show.screenId)?.name || 'N/A'}</td>
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm text-gray-800">{new Date(show.showTime).toLocaleString()}</td>
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm text-gray-800">₹{show.price?.toFixed(2) || '0.00'}</td>
                              <td className="py-2 px-3 sm:px-4 border-b text-xs sm:text-sm">
                                <Link to={`/edit-show/${show.id}`} className="text-blue-600 hover:underline mr-1 sm:mr-2 text-xs sm:text-sm">Edit</Link>
                                <button onClick={() => handleDeleteShow(show.id)} className="text-red-600 hover:underline text-xs sm:text-sm">Delete</button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    <p className="text-gray-600 text-xs sm:text-sm">No shows scheduled for this theatre yet. <Link to="/create-show" className="text-blue-600 hover:underline">Add one</Link>.</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <p className="text-gray-600 text-base sm:text-lg">You don't own any theatres yet. <Link to="/create-theatre" className="text-blue-600 hover:underline font-bold">Create your first theatre!</Link></p>
        )}
      </div>
    </div>
  );
